...
##### 2)
Copy your .ggb file where your points are to the files/read directory (HyLogo.ggb and TestPoints.xml are example files that are already present)
...
##### 5)
Create a new ParticleDataHolder using it's builder subclass (I'll make a list of all the methods later)
##### 6)
Convert the ParticleDataHolder to a ParticleSystem using yourParticleDataHolder.convertToParticleSystem() and write the file using AssetWriter.toJsonFile(yourParticleSpawner)

##### 7)
Find your file under files/write/
...
- make it possible to have multiple attractors in each ParticleSpawerGroup
